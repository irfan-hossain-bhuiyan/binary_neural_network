...
| A2 | 4-bit unsigned addition | Depth × XOR-residual gradient propagation | 3 × (1 + 2×4) configurations | one no-residual depth-4 run reached continuous exact; no residual run reached Boolean exact | exact Boolean recovery: 0/27 | complete; depth/residual diagnostic | `operator_results/a2_depth_gradient_results.json` | `a2_depth_gradient_report.md` |
| A3 | 4-bit unsigned addition | Whole-word rational/softplus tolerance losses | 5 × 4 losses | row-max softplus: 1/5 continuous-exact; baseline 0/5 | exact Boolean recovery: 0/20 | complete; tolerance aggregation diagnostic | `operator_results/a3_tolerance_loss_results.json` | `a3_tolerance_loss_report.md` |
| A4 | 4-bit unsigned addition | MSE prefix → MSE/row-max softplus/row-max rational | 5 × 3 staged branches | no strict continuous-exact run; best final fraction 0.9961 | no exact Boolean recovery; rational final mean 0.5297 | complete; staged tolerance did not solve topology | `operator_results/a4_staged_tolerance_results.json` | `a4_staged_tolerance_report.md` |
| A5 | 4-bit unsigned addition | MSE + λp(1−p), λ=1.5; fixed and staged | 5 × 3 arms | MSE control strongest; staged Gini intermediate | no exact Boolean recovery; staged mean 0.557 | complete; polarization hardened wrong topology | `operator_results/a5_target_gini_results.json` | `a5_target_gini_report.md` |
