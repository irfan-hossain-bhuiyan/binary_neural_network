...
for ax, (label, path) in zip(axes, runs.items()):
    ...
    for key, pretty in [
        ...
        ax.plot(epochs, [entry.get(key, float("nan")) for entry in trajectory],
                label=pretty, color=colors[pretty], linewidth=1.5)
...
fig.suptitle("M001-R vs matched same-depth no-residual control (seed 0)")
