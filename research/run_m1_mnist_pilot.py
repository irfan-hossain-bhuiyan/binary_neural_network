...
CKOUT = OUT / "m1_mnist_pilot_checkpoints"
...
def loss_value(output, target, loss_name, step, total_steps):
    ...
    if loss_name == "BCE":
        value = F.binary_cross_entropy(output.clamp(1e-7, 1 - 1e-7), target)
    else:
        value = (output - target).abs().pow(alpha).mean()
    return value, alpha, phase
...
def run_smoke_and_matrix(args):
    ...
    payload = {"experiment": "M1-mnist-scale-discretization-pilot", "git_sha": git_sha,
               ...
               "smoke_test": smoke, "split_indices_sha256": state_hash({"train": data["train_idx"], "val": data["val_idx"]}),
               "runs": []}
    if args.preflight:
        for loss in LOSSES:
    ...
    for seed in SEEDS:
        for loss in LOSSES:
            print(f"M1 running seed={seed} loss={loss} batch={batch}", flush=True)
            payload["runs"].append(run_one(seed, loss, data, device, batch, epochs=args.epochs))
    return payload


def main():
    p = argparse.ArgumentParser(); p.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu"); p.add_argument("--batch-size", type=int, default=64); p.add_argument("--epochs", type=int, default=20); p.add_argument("--preflight", action="store_true"); p.add_argument("--output", default=str(OUT / "m1_mnist_pilot_results.json")); args = p.parse_args()
